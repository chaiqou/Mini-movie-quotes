<x-admin-layout>
    <section class="max-w-md  mx-auto py-8">
        <div class="flex">
            <main
                class="w-72  justify-center items-center max-w-xl mx-auto mt-10 bg-gray-100  border border-gray-200 p-6 rounded-xl">
                <h1 class="text-blue-700 font-bold text-lg mb-8 border-b border-blue-700">ADMIN DASHBOARD</h1>
                <ul role="list" class="divide-y  divide-gray-300">
                    <li class="py-4 flex">
                        <img class="h-10 w-10 rounded-full"
                            src="https://images.unsplash.com/photo-1598899134739-24c46f58b8c0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2056&q=80"
                            alt="">
                        <div class="ml-3">
                            <a href="movies">All Movie</a>
                            <p class="text-sm text-gray-500">Check all movies</p>
                        </div>
                    </li>

                    <li class="py-4 flex">
                        <img class="h-10 w-10 rounded-full"
                            src="https://images.unsplash.com/photo-1598899134739-24c46f58b8c0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2056&q=80"
                            alt="">
                        <div class="ml-3">
                            <a href="movies/create">New Movie</a>
                            <p class="text-sm text-gray-500">Add new movie here</p>
                        </div>
                    </li>

                    <li class="py-4 flex">
                        <img class="h-10 w-10 rounded-full"
                            src="https://images.unsplash.com/photo-1598899134739-24c46f58b8c0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2056&q=80"
                            alt="">
                        <div class="ml-3">
                            <a href="quotes">All Quote</a>
                            <p class="text-sm text-gray-500">Check all quotes here</p>
                        </div>
                    </li>

                    <li class="py-4 flex">
                        <img class="h-10 w-10 rounded-full"
                            src="https://images.unsplash.com/photo-1598899134739-24c46f58b8c0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2056&q=80"
                            alt="">
                        <div class="ml-3">
                            <a href="quotes/create">New Quote</a>
                            <p class="text-sm text-gray-500">Add new quote here</p>
                        </div>
                    </li>


                </ul>

            </main>
        </div>
    </section>
</x-admin-layout>
